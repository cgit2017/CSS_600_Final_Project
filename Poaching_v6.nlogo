...
to reproduce
  ...
  [
    ...
    let max-tusk [tusk-weight] of max-tusk-elephant * 100
    let log-base-elephant one-of reproducable-elephants with-min [tusk-weight]
    let log-base [tusk-weight] of log-base-elephant * 100
    set chance-of-reproducing log (tusk-weight * 100) log-base
    if chance-of-reproducing > random-float log max-tusk log-base [
      set energy (energy / 2)
